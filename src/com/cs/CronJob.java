...
public class CronJob extends Configured implements Tool {
	...
	public int run(String[] paramArrayOfString) throws Exception {
		...
		final String tBaseLocation = reader.getProperty("BASE_LOCATION");

		final String tFileSourceLocation = tBaseLocation
				+ HDFSUtils.FILE_SEPARATOR + reader.getProperty("LANDING_ZONE");
		// Create directory if does not exist.
		FileUtils.createDirectories(tFileSourceLocation);
		final String tArchiveLocation = tBaseLocation
				+ HDFSUtils.FILE_SEPARATOR + reader.getProperty("ARCHIVE");
		FileUtils.createDirectories(tArchiveLocation);

		final String tFailedLocation = tBaseLocation + HDFSUtils.FILE_SEPARATOR
				+ reader.getProperty("FAILED");

		final String tHDFSBaseLocation = reader
				.getProperty("HDFS_BASE_LOCATION");
		final String tDestinationPath = tHDFSBaseLocation
				+ HDFSUtils.FILE_SEPARATOR
				+ reader.getProperty("HDFS_LANDING_ZONE");
			...
			if (tInboxDir.isDirectory()) {
				...
				for (File tInputFile : tListFiles) {
					String[] args = { tInputFile.getAbsolutePath().toString(),
							tDestinationPath };
					boolean isCopied = HDFSUtils.copyFromLocal(tConf, hdfs,
							args);
					if (isCopied) {
						FileUtils.moveFile(tInputFile, new File(
								tArchiveLocation));
					} else {
						FileUtils.moveFile(tInputFile,
								new File(tFailedLocation));
					}
